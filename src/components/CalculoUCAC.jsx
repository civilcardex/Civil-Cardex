...
import { APARATOS_DEF, AC_UC_IDS, pisoCorto } from "./constants";
...
const { tramosAc, pisos, aps } = useSanitario();
...
const totales = AP.map(d => ({
  id: d.id, nombre: d.nombre, uc: d.uc_ac,
  cant: tramosAc.reduce((s, t) => s + ((t.fixtures?.[d.id] || 0)), 0)
}));
...
<span className="card-t"><img src="/iconos_diseno_redes/RAC_Calculo_UC.webp" alt="" style={{width:24,height:24,verticalAlign:'middle',marginRight:4}} /> C&aacute;lculo Unidades de Consumo Agua caliente</span>
...
<span style={{fontWeight:700,color:'var(--ac)',fontSize:11}}>{subtotal}</span>
</div>
</td>
);
})}
