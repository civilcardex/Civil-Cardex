...
const PdfCanvas_S4: React.CSSProperties = { marginTop:6,padding:'8px 18px', background:'rgba(0,220,229,0.15)',border:'1px solid rgba(0,220,229,0.45)', borderRadius:6,color:'#00dce5',fontWeight:700,fontSize: 12,cursor:'pointer', fontFamily:"'Geist',monospace",letterSpacing:1,textTransform:'uppercase', };
const PdfCanvas_S5: React.CSSProperties = { position:"absolute",top:0,left:0,right:0,bottom:0,zIndex:10, display: "flex", flexDirection: "column", alignItems: "center", justifyContent:"center", gap: 12, padding: 40, background: "#111317" };
const PdfCanvas_S6: React.CSSProperties = { position:'absolute',top:0,left:0,right:0,bottom:0,zIndex:20, display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:14, background:'rgba(17,19,23,0.95)', };
const PdfCanvas_S7: React.CSSProperties = { display:'flex',flexDirection:'column',alignItems:'center',gap:14, padding:'32px 48px',maxWidth:480, background:'linear-gradient(135deg,rgba(245,166,35,0.18),rgba(245,166,35,0.08))', border:'2px solid rgba(245,166,35,0.55)',borderRadius:12, boxShadow:'0 8px 40px rgba(245,166,35,0.2),inset 0 1px 0 rgba(245,166,35,0.15)', };
const PdfCanvas_S8: React.CSSProperties = { marginTop:6,padding:'8px 18px', background:'rgba(0,220,229,0.15)',border:'1px solid rgba(0,220,229,0.45)', borderRadius:6,color:'#00dce5',fontWeight:700,fontSize: 12,cursor:'pointer', fontFamily:"'Geist',monospace",letterSpacing:1,textTransform:'uppercase', };
const PdfCanvas_S9: React.CSSProperties = { position: 'absolute', bottom: 0, left: 0, right: 0, display: "flex", alignItems: "center", gap: 10, padding: "4px 14px", background: "rgba(17,19,23,0.92)", borderTop: "1px solid #3a494a", fontFamily: "'Geist',monospace", fontSize: 12, color: "#8AB4D6", };
    ...
    {(!currentFile || selectedNivel === null || selectedNivel === undefined) && !error && (
      <div style={PdfCanvas_S2}>
        <div style={PdfCanvas_S3}>
          ...
          <button type="button" onClick={() => { sessionStorage.setItem('openTab', 'planos'); navigate('/civilflowareatrabajo'); }} style={PdfCanvas_S4}>📐 Ir a Carga de planos</button>
        </div>
      </div>
    )}
      ...
      {currentFile && !error && selectedNivel !== null && selectedNivel !== undefined && !planos.some((p: { nivel: number; status: string }) => p.nivel === selectedNivel && p.status === 'confirmed') && (
        <div style={PdfCanvas_S6}>
          <div style={PdfCanvas_S7}>
            ...
            <button type="button" onClick={() => { sessionStorage.setItem('openTab', 'planos'); navigate('/civilflowareatrabajo'); }} style={PdfCanvas_S8}>📐 Ir a Carga de planos</button>
          </div>
        </div>
      )}
