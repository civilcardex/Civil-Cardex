...
import { AF_UC_IDS, APARATOS_DEF, pisoCorto } from "../constants";
import { calcUCparcial } from "../utils/componentHelpers";
import { DIAMETROS_AF, COEF_HAZEN_PVC, CONTADORES } from "../utils/calcHydraulics";
import { writeDiametroToDrawing, deleteRamalFromDrawing } from "../utils/writeDiameterToDrawing";
import { LE_ACC_DEF, lookupInterno, calcLeAcces } from "../utils/accesoriosUtils";
...
export default function DisenoRedAguaFria() {
  const { tramosAf, updTramoAf, delTramoAf } = useTramos();
  const { proy } = useProject();
  const { planos } = usePlanos();

  const AF_DIAM_OPTS = useMemo(() => {
    return DIAMETROS_AF.map(d => ({ pulg: d.pulg, nominal: d.nominal, label: d.nominal, dInt: d.dInt }));
  }, []);
  ...
  const handleDiamChange = useCallback((tramoId, nominal) => {
    const opt = AF_DIAM_OPTS.find(o => o.nominal === nominal);
    if (!opt) return;
    const pulg = opt.pulg;
    updTramoAf(tramoId, 'diamDisPulg', pulg);
    ...
    <>
      <div className="card">
        <div className="card-h">
          <span className="card-t"><img src="/iconos_diseno_redes/RAF_Diseno.webp" alt="" style={{width:24,height:24,verticalAlign:'middle',marginRight:4}} /> Diseño de red agua fría</span>
            ...
            <table className="tbl" style={{ fontSize: 11, tableLayout: "auto", width: "100%" }}>
              ...
              <tbody>
                ...
                {tramosOrden.map((t) => {
                  ...
                  const internoMm = diamIntMap[t.id] || lookupInterno(disPulg) || 0;
                        ...
                        <div style={{display:"flex",flexWrap:"nowrap",gap:0,justifyContent:"center",alignItems:"center"}}>
                          {otherTramos.length===0?<span style={{fontSize:9,color:"var(--txt3)"}}>—</span>:otherTramos.map(o=>
                            <button key={o.id} onClick={()=>toggleOtro(t.id,o.id)}
                              style={{fontSize:8,padding:"0 1px",border:"1px solid",borderRadius:1,cursor:"pointer",background:selected.has(o.id)?"var(--af)":"transparent",color:selected.has(o.id)?"#fff":"var(--txt3)",borderColor:selected.has(o.id)?"var(--af)":"var(--line)",lineHeight:1.5}}>{o.id}</button>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="c td-mono">{raizQ>0?fmt(raizQ,2):"—"}</td>
                       <td className="c" style={{padding:"0 1px"}}>
                        <select value={diamNomMap[t.id] || ''} onChange={e=>handleDiamChange(t.id, e.target.value)}
                          ...
                          <option value="">—</option>
                          {AF_DIAM_OPTS.map(o=><option key={o.nominal} value={o.nominal}>{o.label}</option>)}
                        </select>
                      </td>
          ...
          <table className="tbl" style={{ width: "100%", fontSize: 10, tableLayout: "fixed" }}>
            ...
            <tbody>
              ...
              {tramosOrden.map((t) => {
                ...
                return (
                  <tr key={t.id}>
                    <td className="c" style={{  fontWeight: 700, padding: "4px 2px", textAlign: "center", background: Le > 0 ? "rgba(59,130,246,.12)" : "transparent" }}>
                      {Le > 0 ? fmt(Le, 2) : "—"}
                    </td>
