...
export function renderBajantes(ctx: CanvasRenderingContext2D, engine: IPlanoEngineCore): void {
  engine.bajantes.forEach((b: any) => {
    ...
    if (sel) {
      ...
      const fsDir = engine.mm2cvs(engine.MM.lblInfo);
      const lineH = fsCode + 2;
      // Build line1: BAN #1:4"
      const dInches = b.dNominal ? Math.round(Number(b.dNominal) / 25.4) : 0;
      const line1 = dInches > 0 ? `${b.code}:${dInches}"` : (b.code || '—');
      ...
      const boxW = tw1 + engine.mm2cvs(3);
      ...
      // Background
      ctx.fillStyle = '#ffffff';
      ctx.fillRect(-boxW / 2, -10, boxW, boxH);
      ...
      ctx.lineWidth = 0.5;
      ctx.beginPath();
      ctx.moveTo(-boxW / 2 + 2, sepY);
      ctx.lineTo(boxW / 2 - 2, sepY);
      ctx.stroke();
