...
export function ExcelPreviewModal({ excel }: { excel: ExcelHandle }) {
  ...
  return (
    <div className="cm-modal-overlay" role="presentation" onClick={excel.doImportCancel}>
      <div className="cm-modal-box cm-modal-wide" role="dialog" aria-modal="true" aria-label={isExport ? 'Previsualizar exportación' : 'Previsualizar importación'} onClick={e => e.stopPropagation()}>
        <div className="cm-modal-head cm-modal-head-row">
          <span>{isExport ? 'Previsualizar Exportación' : 'Previsualizar Importación'}</span>
          <button type="button" className="cm-btn-icon" onClick={excel.doImportCancel} aria-label="Cerrar">
          ...
          <table className="cm-tbl">
            <thead>
              <tr>
                ...
                {pm.headers.map((h, i) => <th key={i}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {pm.rows.length === 0 && (
                <tr>
                  <td colSpan={pm.headers.length + 1} className="cm-empty-row">Sin datos</td>
                </tr>
              )}
              {pm.rows.map((row, ri) => (
                <tr key={ri}>
        ...
        <div className="cm-modal-actions">
          ...
          <button type="button" className="cm-btn" onClick={excel.doImportCancel}>Cancelar</button>
        </div>
      </div>
    </div>
  );
