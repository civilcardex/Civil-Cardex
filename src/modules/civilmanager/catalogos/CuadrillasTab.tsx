...
export function CuadrillasTab() {
  ...
  const filtered = search
    ? state.cuadrillas.filter((c) =>
        (c.codigo + c.descripcion).toLowerCase().includes(search.toLowerCase()),
      )
    : state.cuadrillas;
  ...
  function addCuadrilla() {
    const nueva: Cuadrilla = {
      ...
      codigo: genCodeFor(state.cuadrillas, 'CU'),
      descripcion: 'Nueva cuadrilla',
      integrantes: [],
    };
  ...
  function addIntegrante() {
    ...
    const nuevo: CuadrillaIntegrante = {
      id: crypto.randomUUID(),
      cargo_id: cargosCalc[0].id,
      cantidad: 1,
    };
  ...
  const cost = sel
    ? calcCuadrillaCost(sel, cargoMap, state.config.dias_mes, state.config.horas_mes, esHora)
    : null;
          ...
          <XlWrap>
            ...
            >
              ...
              <span style={{ fontSize: 11, color: 'var(--txt2)' }}>
                Costo/día: <b>{fmt(cost?.costoDia ?? 0)}</b> · Costo/hora:{' '}
                <b>{fmt(cost?.costoHora ?? 0)}</b>
              </span>
            </div>
            <XlScroll>
              <table>
