...
function Field({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2, minWidth: 180 }}>
      <label htmlFor={id} style={{ fontSize: 10, color: 'var(--txt2)', fontWeight: 700 }}>
        {label}
      </label>
      {control}
    ...
    >
      ...
      <Field label="Auxilio de transporte">
        <NumInput
          ...
          format
          onChange={(v) => upd('auxilio_transporte', v)}
        />
      </Field>
      ...
      <Field label="Administración %" help={c.administracion}>
        <NumInput
          value={config.pct_administracion}
          decimals={2}
          onChange={(v) => upd('pct_administracion', v)}
        />
      </Field>
      <Field label="Imprevistos %" help={c.imprevistos}>
        <NumInput
          value={config.pct_imprevistos}
          decimals={2}
          onChange={(v) => upd('pct_imprevistos', v)}
        />
      </Field>
      <Field label="Utilidad %" help={c.utilidad}>
        <NumInput
          value={config.pct_utilidad}
          decimals={2}
          onChange={(v) => upd('pct_utilidad', v)}
        />
      </Field>
      <Field
        label="Usar F.P. en cada APU"
        help={config.usar_fp_en_apu ? undefined : c.costo_personal_fp}
      >
        <select
          className="cm-sel"
          value={config.usar_fp_en_apu ? 'si' : 'no'}
          onChange={(e) => upd('usar_fp_en_apu', e.target.value === 'si')}
        >
      ...
      <Field label="Aplicar AIU por APU" help={c.usar_en_cada_apu}>
        <select
          className="cm-sel"
          value={config.usar_en_cada_apu ? 'si' : 'no'}
          onChange={(e) => upd('usar_en_cada_apu', e.target.value === 'si')}
        >
      ...
      <Field label="Valor resumido" help={c.vr_resumido}>
        <select
          className="cm-sel"
          value={config.vr_resumido ? 'si' : 'no'}
          onChange={(e) => upd('vr_resumido', e.target.value === 'si')}
        >
