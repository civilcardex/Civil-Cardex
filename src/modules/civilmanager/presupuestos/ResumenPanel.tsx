...
export function ResumenPanel({ pres, onUpdate }: Props) {
  ...
  return (
    <div>
      <fieldset
        className="cm-xl-wrap"
        disabled={!editable}
        style={{ padding: 14, margin: 0, border: 0, minWidth: 0 }}
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
            <input
              type="checkbox"
              checked={aiu.activo}
              onChange={(e) => updAiu('activo', e.target.checked)}
            />
        ...
        {aiu.activo && (
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            <div>
              <label htmlFor="aiu-pct-a" style={{ fontSize: 10, color: 'var(--txt2)' }}>
                Administración %
              </label>
              <NumInput
                id="aiu-pct-a"
                value={aiu.pct_a}
                decimals={2}
                onChange={(v) => updAiu('pct_a', v)}
              />
            </div>
            <div>
              <label htmlFor="aiu-pct-i" style={{ fontSize: 10, color: 'var(--txt2)' }}>
                Imprevistos %
              </label>
              <NumInput
                id="aiu-pct-i"
                value={aiu.pct_i}
                decimals={2}
                onChange={(v) => updAiu('pct_i', v)}
              />
            </div>
            <div>
              <label htmlFor="aiu-pct-u" style={{ fontSize: 10, color: 'var(--txt2)' }}>
                Utilidad %
              </label>
              <NumInput
                id="aiu-pct-u"
                value={aiu.pct_u}
                decimals={2}
                onChange={(v) => updAiu('pct_u', v)}
              />
            </div>
            <div>
              <label htmlFor="aiu-iva-pct" style={{ fontSize: 10, color: 'var(--txt2)' }}>
                IVA sobre utilidad %
              </label>
              <NumInput
                id="aiu-iva-pct"
      ...
      <div className="cm-xl-wrap" style={{ padding: 14 }}>
        <table className="cm-tbl">
          <tbody>
            <tr>
              <td>Costo Directo</td>
              <td style={{ textAlign: 'right' }}>{fmt(resumen.costoDirecto)}</td>
            </tr>
            <tr>
              <td>Administración</td>
              <td style={{ textAlign: 'right' }}>{fmt(resumen.administracion)}</td>
            </tr>
            <tr>
              <td>Imprevistos</td>
              <td style={{ textAlign: 'right' }}>{fmt(resumen.imprevistos)}</td>
            </tr>
            <tr>
              <td>Utilidad</td>
              <td style={{ textAlign: 'right' }}>{fmt(resumen.utilidad)}</td>
            </tr>
            <tr>
              <td>IVA sobre utilidad</td>
              <td style={{ textAlign: 'right' }}>{fmt(resumen.ivaUtilidad)}</td>
            </tr>
