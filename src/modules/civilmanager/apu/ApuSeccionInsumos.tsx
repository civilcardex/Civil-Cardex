...
export function ApuSeccionInsumos({ apu, onChange }: Props) {
  ...
  function add() {
    ...
    onChange([
      ...apu.recursos_ins,
      { insumo_id: state.insumos[0].id, consumo: 1, desperdicios_pct: 5 },
    ]);
  }
  ...
  function costoUnitario(insumoId: string): number {
    ...
    if (ins.origen === 'Preparado en obra')
      return abMap.get(ins.apu_basico_id)?.costo_unitario ?? 0;
                ...
                <tr key={filaId(r)}>
                  ...
                  <td>
                    <select
                      className="cm-sel"
                      value={r.insumo_id}
                      onChange={(e) => upd(i, 'insumo_id', e.target.value)}
                      aria-label="Insumo"
                    >
                      {state.insumos.map((x) => (
                  ...
                  <td>
                    <NumInput
                      value={r.consumo}
                      decimals={4}
                      onChange={(v) => upd(i, 'consumo', v)}
                    />
                  </td>
                  <td>
                    <NumInput
                      value={r.desperdicios_pct}
                      decimals={2}
