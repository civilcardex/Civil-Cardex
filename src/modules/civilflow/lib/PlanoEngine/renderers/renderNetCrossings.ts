...
NETS.forEach((n, i) => { NET_ORDER[n.id] = i; });

interface Hit { x: number; y: number }
...
export function renderNetCrossings(ctx: CanvasRenderingContext2D, engine: IPlanoEngineCore): void {
  const ramales = engine.ramales.filter((r) =>
      ...
      for (let a = 0; a < ra.pts.length - 1; a++) {
        for (let b = 0; b < rb.pts.length - 1; b++) {
          const hit = segCrossing(
            ra.pts[a][0], ra.pts[a][1], ra.pts[a + 1][0], ra.pts[a + 1][1],
            rb.pts[b][0], rb.pts[b][1], rb.pts[b + 1][0], rb.pts[b + 1][1]
          );
          if (!hit) continue;
          ...
          const jSeg: [number[], number[]] = raIsJumper ? [ra.pts[a], ra.pts[a + 1]] : [rb.pts[b], rb.pts[b + 1]];
          const oSeg: [number[], number[]] = raIsJumper ? [rb.pts[b], rb.pts[b + 1]] : [ra.pts[a], ra.pts[a + 1]];
