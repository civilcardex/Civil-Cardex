...
export interface GasDatosGenerales {
  alt: string; patm: string; temp: string; pmin: string; densRel: string;
}

export const GAS_DATOS_DEFAULT: GasDatosGenerales = { alt: '959', patm: '90.32', temp: '23', pmin: '17', densRel: '0.67' };

let allDnCache: { mat: string; K: number; dn: string; d: number }[] | null = null;
function allDn() {
  if (!allDnCache) {
    allDnCache = [];
    GAS.forEach(g => g.rows.forEach(r => allDnCache!.push({ mat: g.mat, K: g.K, dn: r.dn, d: r.d })));
  }
  return allDnCache;
}
...
export interface GasRow {
  id: string; piso: number; ini: string; fin: string;
  material: string; dn: string; dInt: number; K: number;
  longitud: number; le: number; dP: number; vel: number;
  pIni: number; pFin: number; chequeo: string;
}
...
export function computeGasRows(plans: PlanItem[]): GasRow[] {
  ...
  const gasTramos: { id: string; planId: string | number; piso: number; ini: string; fin: string; longitud: number; material: string; diametro: string }[] = [];
  for (const plano of plans) {
    ...
    for (const r of (data.ramales || []) as GasRamalRaw[]) {
      ...
      gasTramos.push({
        id: r.id, planId: plano.id, piso: Number(r.piso ?? plano.nivel) || 0,
        ini: r.ini || '', fin: r.fin || '', longitud: r.totalL || r.Lh || 0,
        material: r.material || '', diametro: r.diametro || '',
      });
    }
  }
  ...
  const aparatos = loadFromStorage<Record<string, Record<string, number>>>(APARATOS_BY_TRAMO_KEY, {});
  ...
  for (const t of gasTramos) {
    ...
    const dP = dInt > 0 ? 23200 * (le + t.longitud) * Math.pow(qDiseno, 1.82) / Math.pow(dInt, 4.82) * Math.pow(DR, 0.82) : 0;
    const vel = dInt > 0 ? 354 * qDiseno * 101.325 / (dInt * dInt) / pAtm : 0;
    const pIni = pAcum;
    const pFin = pAcum - dP;
    pAcum = pFin;
    const ok = (vel > 0 && vel <= 10 && dP > 0) ? 'O.K.' : (dP > 0 ? 'NO' : '—');
    result.push({ id: t.id, piso: t.piso, ini: t.ini, fin: t.fin, material: t.material, dn, dInt, K, longitud: t.longitud, le, dP, vel, pIni, pFin, chequeo: ok });
  }
  return result;
}
