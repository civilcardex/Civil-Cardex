...
function calculateVentStack(params: {
  UD_acum: number; r: number; n: number; bajDprop: number; bajLong: number; bajFDarcy: number; ventDprop: number;
}) {
  ...
  const D_vent_calc_pulg = bajLong > 0 && Q_aire > 0 ? Math.pow((bajLong * fDarcy * Q_aire * Q_aire) / 3.25, 1 / 5) : 0;
...
export function computeBajanteVentTable(tramosSan: Tramo[], plans: PlanItem[], udBase: UDBase[], pisos: { n: number }[]): MemoriaTable | null {
  ...
  const getDescendantsUD = (tKey: string, visited = new Set<string>()): number => {
    ...
    for (const childKey of children) {
      ...
      if (childTramo) sum += calcUDparcial(childTramo, udBase) + getDescendantsUD(childKey, visited);
    }
    return sum;
  };
  ...
  const getBajanteTotalUD = (bKey: string, visited = new Set<string>()): number => {
    ...
    for (const otherB of tramosSan) {
      ...
      if (otherB.descargaEnId) {
        ...
        if (matches) sum += getBajanteTotalUD(otherB._key || `${otherB.id}-${otherB.planId}`, visited);
      }
    }
    return sum;
  };
  ...
  const headers = [
    'No.', 'Origen', 'Destino', 'Ramales asociados', 'Unidades descarga', 'Llenado', 'Caudal (LPS)', 'Manning',
    'D calculado (")', 'D propuesto (")', 'Chequeo', 'Caudal máximo (LPS)', 'Vel. terminal (m/s)', 'L terminal calc (m)', 'L terminal mín (m)',
    'Vel. aire (m/s)', 'Fricción (f)', 'Caudal aire (LPS)', 'Longitud bajante (m)', 'D vent calc (")', 'D vent propuesto (")',
  ];
  ...
  const rows = banTramos.map(t => {
    ...
    if (!isVent) {
      ...
      for (const sk of sanBajKeys) {
        ...
        if (st) { resolvedSanDprop = st.bajDprop || 0; break; }
      }
    }
    ...
    return [
      t.code || t.id, origenVal, destinoVal, ramalesAsocVal,
      totalUD > 0 ? totalUD : '—', rStr || '—', res.Q_Ls > 0 ? res.Q_Ls.toFixed(2) : '—', n > 0 ? n.toFixed(3) : '—',
      res.Dcalc_pulg > 0 ? res.Dcalc_pulg.toFixed(2) + '"' : '—', res.Dprop_pulg > 0 ? res.Dprop_pulg + '"' : '—', res.chequeoDiam,
      res.QmaxBajante > 0 ? res.QmaxBajante.toFixed(2) : '—', res.Vt > 0 ? res.Vt.toFixed(2) : '—',
      res.Lt_calc > 0 ? res.Lt_calc.toFixed(2) : '—', res.Lt_min > 0 ? res.Lt_min.toFixed(2) : '—',
      res.V_aire > 0 ? res.V_aire.toFixed(2) : '—', (t.bajFDarcy ?? 0) > 0 ? (t.bajFDarcy ?? 0).toFixed(3) : '—',
      res.Q_aire_Ls > 0 ? res.Q_aire_Ls.toFixed(2) : '—', t.bajLong ?? 3,
      res.D_vent_calc_pulg > 0 ? res.D_vent_calc_pulg.toFixed(2) + '"' : '—', res.D_vent_prop_pulg > 0 ? res.D_vent_prop_pulg + '"' : '—',
    ];
  });
