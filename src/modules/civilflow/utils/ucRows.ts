...
const TIPO_CFG = {
  af: { ucIds: AF_UC_IDS, field: 'uc_af' as const, apField: 'ucaf' as const, title: 'agua fría', showTotal: true },
  ac: { ucIds: AC_UC_IDS, field: 'uc_ac' as const, apField: 'ucac' as const, title: 'agua caliente', showTotal: false },
};

export function computeUcTable(tipo: 'af' | 'ac', tramos: Tramo[], aps: ApsItem[]): MemoriaTable | null {
  ...
  const sortedTramos = tramos.toSorted((a, b) => (a.piso || 0) - (b.piso || 0));

  const headers = ['Tramo', 'Nivel', 'Inicio', 'Fin', ...AP.map(d => `${d.nombre} (${d[field]} UC)`), 'Parcial'];
  if (showTotal) headers.push('Total', 'Longitud (m)', 'No. desc. simult.');

  const headerGroups: (string | MemoriaHeaderGroup)[] = ['Tramo', 'Nivel', 'Inicio', 'Fin', { label: 'Aparatos', span: AP.length }];
  if (showTotal) headerGroups.push({ label: 'Unidades de consumo', span: 2 }, 'Longitud (m)', 'No. desc. simult.');
  ...
  const rows = sortedTramos.map(t => {
    ...
    const row: (string | number)[] = [t.id, pisoLbl(t.piso), ini, fin, ...AP.map(d => t.fixtures?.[d.id] || 0), parcial];
    if (showTotal) {
  ...
  const sumRow: (string | number)[] = ['Σ', '', '', '', ...totales.map(d => `${d.cant} × ${d.uc} UC`)];
