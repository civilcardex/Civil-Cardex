...
const ViewerPage_S4: React.CSSProperties = { position: 'absolute', top: '100%', left: 0, marginTop: 4, minWidth: 240, background: '#15171b', border: '1px solid #3a494a', borderRadius: 6, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', overflow: 'hidden', zIndex: 100, };
const ViewerPage_S5: React.CSSProperties = { flex: 1, padding: '5px 8px', background: '#1e2024', border: '1px dashed #3a494a', borderRadius: 3, color: '#b9caca', cursor: 'pointer', fontSize: 12, fontFamily: 'Geist, monospace', fontWeight: 600, textAlign: 'center', };
const ViewerPage_S6: React.CSSProperties = { padding: '5px 8px', background: '#1e2024', border: '1px solid #3a494a', borderRadius: 3, color: '#9BA8AA', cursor: 'pointer', fontSize: 12, fontFamily: 'Geist, monospace', textAlign: 'center', };
          ...
          <nav ref={dropdownNavRef} aria-label="Selector de planos" style={{ position: 'relative' }}>
            ...
            {dropdownOpen && (
              <div role="listbox" aria-label="Seleccionar plano" style={ViewerPage_S4}>
                {plans.length===0&&(
                  <div style={{padding:'12px 16px',color:'#8AB4D6',fontFamily:'Geist, monospace',fontSize: 12,textAlign:'center'}}>
                ...
                {plans.map((p, i) => (
                  <div key={p.id}
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      <span style={{
                        fontFamily: 'Geist, monospace', fontSize: 12, color: '#e2e2e8',
                    ...
                    {i === activeIndex && (
                      <span style={{ fontFamily: 'Geist, monospace', fontSize: 12, color: '#00dce5' }}>●</span>
                    )}
                  </div>
                ))}
                <div style={{ borderTop: '1px solid #3a494a', padding: '6px 12px', display: 'flex', gap: 8 }}>
                  ...
                  <button type="button" onClick={() => navigate('/civilflowareatrabajo')}
                    style={ViewerPage_S6}>
                    📐 Planos
                  </button>
                </div>
