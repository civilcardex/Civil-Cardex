...
const FixtureGrid_S4: React.CSSProperties = { flex: 1, padding: '2px 0', background: 'transparent', fontSize: 12, fontWeight: 800, fontFamily: "'Geist',monospace", border: 'none' } as const;
const FixtureGrid_S5: React.CSSProperties = { flex: 1.2, textAlign: 'center', fontSize: 12, fontWeight: 800, lineHeight: '14px', fontFamily: "'Geist',monospace" } as const;
      ...
      {items.map(ap => {
        ...
        return (
          <div key={ap.id}
            ...
            style={{ ...FixtureGrid_S2, background: active ? 'rgba(37,99,235,.12)' : 'var(--bg2)', border: `1px solid ${active ? accent : 'var(--line)'}` }}>
            <button type="button" onClick={() => targetId && inc(ap.id)} disabled={!targetId}
              ...
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: .3, color: active ? accent : '#b9caca', fontFamily: "'Geist',monospace", textTransform: 'uppercase' }}>{abbr}</span>
              <span style={FixtureGrid_S1}>{uStr} {unidadLbl}</span>
            </button>
            <div style={{ display: 'flex', alignItems: 'stretch', borderTop: `1px solid ${active ? accent + '55' : 'var(--bg4)'}`, background: active ? 'rgba(37,99,235,.06)' : 'transparent' }}>
              <button type="button" onClick={(e) => { e.stopPropagation(); if (targetId) dec(ap.id); }} disabled={!targetId || c === 0}
                style={{ ...FixtureGrid_S4, color: c === 0 || !targetId ? 'var(--line)' : '#ffb4ab', cursor: c === 0 || !targetId ? 'not-allowed' : 'pointer', borderRight: `1px solid ${active ? accent + '55' : 'var(--bg4)'}` }}>−</button>
              <div style={{ ...FixtureGrid_S5, color: c > 0 ? accent : 'var(--txt2)', background: c > 0 ? 'rgba(37,99,235,.18)' : 'transparent' }}>{c}</div>
              <button type="button" onClick={(e) => { e.stopPropagation(); if (targetId) inc(ap.id); }} disabled={!targetId}
                style={{ ...FixtureGrid_S4, color: !targetId ? 'var(--line)' : accent, cursor: !targetId ? 'not-allowed' : 'pointer', borderLeft: `1px solid ${active ? accent + '55' : 'var(--bg4)'}` }}>+</button>
            </div>
          </div>
        );
      })}
