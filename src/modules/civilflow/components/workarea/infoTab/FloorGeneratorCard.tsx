...
const FloorGeneratorCard_cubiertaToggle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 6, userSelect: 'none', padding: '4px 8px', borderRadius: 4, flexShrink: 0, border: 'none', background: 'transparent', font: 'inherit', color: 'inherit', width: '100%', textAlign: 'inherit' };
const FloorGeneratorCard_generarBtn: React.CSSProperties = { width: '100%', padding: '6px', marginTop: 6, background: 'var(--acc)', border: 'none', borderRadius: 'var(--r)', color: '#fff', fontWeight: 600, fontSize: 12 };
      ...
      <div className="card-h" style={{ padding: '4px 8px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
          <h3 className="card-t" style={{ fontSize: 13, display: 'flex', alignItems: 'center' }}>
            <img src="/iconos_civilflow/info_general/generador_de_pisos.webp" alt="Generador de pisos"  width={22} height={22} style={{width:22,height:22, verticalAlign: 'middle', marginRight: 2 }}  loading="lazy" />
            Generador de pisos
        ...
        <div style={{ padding: '4px 6px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3, alignItems: 'end' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end', paddingBottom: 2 }}>
              <button type="button" disabled={!isEditing} role="switch" aria-checked={props.conCubierta} onClick={() => props.setConCubierta(!props.conCubierta)} title="Incluir cubierta" style={{ ...FloorGeneratorCard_cubiertaToggle, cursor: isEditing ? 'pointer' : 'default', opacity: isEditing ? 1 : 0.7 }}>
          ...
          <button type="button" disabled={!isEditing} onClick={props.generarPisos} style={{ ...FloorGeneratorCard_generarBtn, cursor: isEditing ? 'pointer' : 'default', opacity: isEditing ? 1 : 0.5 }}>Generar niveles automáticamente</button>
        </div>
    </section>
  );
});
