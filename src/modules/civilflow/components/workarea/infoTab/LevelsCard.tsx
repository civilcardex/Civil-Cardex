...
const LevelsCard_S1: React.CSSProperties = { padding: '3px 6px', background: 'transparent', border: '1px solid var(--line)', borderRadius: 2, color: 'var(--txt3)', cursor: 'pointer', fontSize: 11, lineHeight: 1, flexShrink: 0, marginLeft: 2 };
const LevelsCard_pisoLi: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: 3, padding: '2px 4px', background: 'var(--bg3)', border: '1px solid var(--line)', borderRadius: 'var(--r)', marginBottom: 2 };
      ...
      <div className="card-h" style={{ padding: '4px 8px', flexShrink: 0 }}>
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
          <h3 className="card-t" style={{ fontSize: 13, display: 'flex', alignItems: 'center' }}>
            <img src="/iconos_civilflow/info_general/niveles_generados.webp" alt="Niveles generados"  width={22} height={22} style={{width:22,height:22, verticalAlign: 'middle', marginRight: 2 }}  loading="lazy" />
            Niveles generados
          ...
          <>
            <ul role="list" style={{ flex: 1, overflowY: 'auto', minHeight: 0, listStyle: 'none', margin: 0, padding: 0 }}>
              {pisos.toSorted((a, b) => (b.tipo === 'cubierta' ? 999 : b.n) - (a.tipo === 'cubierta' ? 999 : a.n)).map((p) => (
                <li key={p.id} style={{ ...LevelsCard_pisoLi, borderLeft: '3px solid ' + (p.tipo === 'cubierta' ? '#ffffff' : p.n < 0 ? 'var(--txt3)' : 'var(--acc2)') }}>
                  ...
                  {isEditing && <button type="button" onClick={() => delPiso(p.id)} title="Eliminar nivel" style={LevelsCard_S1} onMouseEnter={e => { e.currentTarget.style.color = '#ef5350'; e.currentTarget.style.borderColor = 'rgba(211,47,47,.5)'; }} onMouseLeave={e => { e.currentTarget.style.color = 'var(--txt3)'; e.currentTarget.style.borderColor = 'var(--line)'; }}>&#x2715;</button>}
                </li>
              ))}
            </ul>
