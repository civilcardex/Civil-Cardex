...
export type IsoCanvas = HTMLCanvasElement & {
  __isoSegments?: IsoSegment[];
  __isoCx?: number;
  __isoCy?: number;
};
...
interface UseIsometriaInteractionParams {
  ...
  offX: number;
  setOffX: (v: number | ((prev: number) => number)) => void;
  offY: number;
  setOffY: (v: number | ((prev: number) => number)) => void;
...
export function useIsometriaInteraction({
  ...
  // Deliberadamente corre en cada render (dragRef es una ref, no una dependencia reactiva) —
  // setState no hace nada cuando el valor no cambió, así que no es un bucle infinito, solo
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (hit) {
        setSelTramo((prev) => (prev === hit ? null : hit));
        return;
      }
      setSelTramo(null);
      dragRef.current = {
