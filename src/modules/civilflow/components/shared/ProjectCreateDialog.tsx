...
export default function ProjectCreateDialog({ open, onClose }: Props) {
  ...
  async function handleCreate() {
    ...
    try {
      const now = new Date()
      const codigo = 'PR-' + now.getFullYear() +
        String(now.getMonth() + 1).padStart(2, '0') +
        String(now.getDate()).padStart(2, '0') + '-' +
        String(now.getHours()).padStart(2, '0') +
        String(now.getMinutes()).padStart(2, '0')
      const proyecto = await createProyecto(codigo, trimmed)
      if (!proyecto) { devError('Error creando proyecto'); setCreating(false); return }
      ...
      <div style={{
        ...
        <input
          ...
          style={{
            ...
            border: '1px solid var(--outline-variant, #3a3a44)',
            borderRadius: 4, color: 'var(--on-surface, #e2e2e8)',
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 16 }}>
          ...
          <button type="button"
            ...
            style={{
              padding: '6px 14px', fontSize: 12, fontWeight: 600,
              background: !name.trim() || creating ? 'var(--surface-container-low, #141418)' : 'var(--primary, #4D8FF7)',
              border: 'none', borderRadius: 4,
              color: !name.trim() || creating ? 'var(--on-surface-variant, #9ba8aa)' : 'var(--on-primary, #fff)',
