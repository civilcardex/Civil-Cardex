...
export default function EPVerificationPage({
  ...
  ep,
  updEP,
}: EPVerificationPageProps) {
  const qac = dec(ep.qac),
    qasc = dec(ep.qasc);
  const hfac = dec(ep.hfac),
    hfacs = dec(ep.hfacs),
    hfotros = dec(ep.hfotros);
  const pred = dec(ep.pred),
    pmin = dec(ep.pmin),
    pmax = dec(ep.pmax);
  ...
  const fs = dec(ep.fs) || 1.15,
    ciclos = dec(ep.ciclos) || 6;
  const alfa = dec(ep.alfa) || 0.3;
  const vsuc = dec(ep.vsuc) || 1.5,
    vimp = dec(ep.vimp) || 2.0;
          ...
          >
            <Tbl
              ...
              rows={[
                ...
                [
                  'Eficiencia motor (η_m)',
                  <LazyInp
                    ep={ep}
                    updEP={updEP}
                    disabled={!editParams}
                    field="etam"
                    ariaLabel="Eficiencia motor"
                  />,
                  'dec',
                ],
                [
                  'Factor de seguridad potencia',
                  <LazyInp
                    ep={ep}
                    updEP={updEP}
                    disabled={!editParams}
                    field="fs"
                    ariaLabel="Factor de seguridad potencia"
                  />,
                  'dec',
                ],
                [
                  <span style={{ fontWeight: 600 }}>
                    {`Ciclos/hora (n)  `}
                    {ciclos > 10 ? (
                      <span style={{ fontWeight: 700, color: '#ef5350' }}>No O.K.</span>
                    ) : (
                      <span style={{ fontWeight: 700, color: '#22c55e' }}>OK</span>
                    )}
                  </span>,
                  <LazyInp
                    ep={ep}
                    ...
                    disabled={!editParams}
                    field="ciclos"
                    ariaLabel="Ciclos por hora"
                  />,
                  'arr/h',
                ],
                [
                  'Fracción útil tanque (α)',
                  <LazyInp
                    ep={ep}
                    updEP={updEP}
                    disabled={!editParams}
                    field="alfa"
                    ariaLabel="Fracción útil tanque"
                  />,
                  'dec',
                ],
                [
                  'Velocidad succión (V_suc)',
                  <LazyInp
                    ep={ep}
                    updEP={updEP}
                    disabled={!editParams}
                    field="vsuc"
                    ariaLabel="Velocidad succión"
                  />,
                  'm/s',
                ],
                [
                  'Velocidad impulsión (V_imp)',
                  <LazyInp
                    ep={ep}
                    updEP={updEP}
                    disabled={!editParams}
                    field="vimp"
                    ariaLabel="Velocidad impulsión"
                  />,
                  'm/s',
                ],
              ]}
            ...
            >
              <div style={{ fontSize: 10, color: 'var(--txt3)' }}>
                ...
                <strong style={{ color: 'var(--txt)' }}>
                  {Pins_hp > 0 ? Pins_hp.toFixed(2) : '—'} HP
                </strong>{' '}
                · Comercial superior:{' '}
                <strong style={{ color: 'var(--txt)', fontWeight: 700 }}>{autoNema} HP</strong>
              </div>
              <select
                aria-label="Potencia comercial"
                disabled={!editPComercial}
                value={ep.pcomercial || ''}
                onChange={(e) => updEP('pcomercial', e.target.value)}
                style={{
                  ...EPVerificationPage_selStyle,
                  fontSize: 10,
                  padding: '4px 6px',
                  cursor: editPComercial ? 'pointer' : 'default',
                  opacity: editPComercial ? 1 : 0.7,
                }}
              >
                <option value="">Seleccione</option>
                {COMM_HP.map(({ hp, kw }) => (
                  <option key={hp} value={String(hp)}>
                    {hp} HP ({kw} kW)
                  </option>
                ))}
              </select>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                ...
                <LazyInp
                  ...
                  disabled={!editPComercial}
                  field="pcomercial"
                  ariaLabel="Potencia comercial"
                  style={{ ...SI, width: 46, fontSize: 10, padding: '2px 4px' }}
                />
                <span
                  style={{
                    fontFamily: 'var(--mono)',
        ...
        >
          <Tbl
            ...
            rows={[
              [
                'Presión de arranque',
                <span style={M}>{fmtMca(Pon)}</span>,
                'm.c.a.',
                'P_arranque = HMT',
              ],
              [
                'Presión de paro',
                <span style={M}>{fmtMca(Poff)}</span>,
                'm.c.a.',
                'P_paro = P_arranque × 1.10',
              ],
              [
              ...
              [
                ...
                <span style={M}>{fmtBar(Poff_bar)}</span>,
                'bar',
                'P_paro = P_arranque × 1.10',
              ],
              [
                'Precarga de nitrógeno N₂ = 0.90 × P_arranque',
                <span style={M}>{fmtBar(PN2_bar)}</span>,
                'bar',
                'P_precarga = 0.90 × P_arranque',
              ],
              [
                <span style={{ fontWeight: 600 }}>Volumen útil Vu = Qd × 60 / (4 × n)</span>,
                <span style={M}>{fmtL(Vu)}</span>,
                'L',
                'Vu = Qd × 60 / (4 × n)',
              ],
              [
                <span style={{ fontWeight: 700, color: 'var(--txt)' }}>
                  Volumen total tanque Vt = Vu / α
                </span>,
                <span style={MB}>{fmtL(Vt)}</span>,
                'L',
                'Vt = Vu / α',
              ],
              [
        ...
        >
          <Tbl
            ...
            rows={[
              [
                'Succión colector (Qd)',
                <span style={M}>{fmtLps(Qd)}</span>,
                <span style={M}>{vsuc}</span>,
                <span style={M}>{fmtMm(rSucColector.diamCalcMm)}</span>,
                <span style={MB}>{rSucColector.dn}</span>,
                <span style={M}>{fmtMs(rSucColector.vReal)}</span>,
              ],
              [
                'Impulsión colector (Qd)',
                <span style={M}>{fmtLps(Qd)}</span>,
                <span style={M}>{vimp}</span>,
                <span style={M}>{fmtMm(rImpColector.diamCalcMm)}</span>,
                <span style={MB}>{rImpColector.dn}</span>,
                <span style={M}>{fmtMs(rImpColector.vReal)}</span>,
              ],
              [
                'Succión por bomba (Qb)',
                <span style={M}>{fmtLps(Qb)}</span>,
                <span style={M}>{vsuc}</span>,
                <span style={M}>{fmtMm(rSucBomba.diamCalcMm)}</span>,
                <span style={MB}>{rSucBomba.dn}</span>,
                <span style={M}>{fmtMs(rSucBomba.vReal)}</span>,
              ],
              [
                'Impulsión por bomba (Qb)',
                <span style={M}>{fmtLps(Qb)}</span>,
                <span style={M}>{vimp}</span>,
                <span style={M}>{fmtMm(rImpBomba.diamCalcMm)}</span>,
                <span style={MB}>{rImpBomba.dn}</span>,
                <span style={M}>{fmtMs(rImpBomba.vReal)}</span>,
              ],
            ]}
          />
        </Card>
          ...
          <Tbl
            ...
            rows={[
              [
                'Caudal nominal',
                <span style={M}>
                  {fmtLps(Qd)} L/s = {fmtM3h(Qm3h)} m³/h = {fmtGpm(Qgpm)} GPM
                </span>,
              ],
              [
              ...
              [
                ...
                <span style={M}>
                  {nt + dec(ep.nr)} uds: {nt} trabajo + {dec(ep.nr)} reserva · Qb = {fmtLps(Qb)} L/s
                  c/u
                </span>,
              ],
              [
                'Potencia comercial',
                <span style={M}>
                  {nemaSel} HP = {(nemaSel * 0.7457).toFixed(0)} kW (calc:{' '}
                  {Pins_hp > 0 ? Pins_hp.toFixed(2) : '—'} HP)
                </span>,
              ],
              [
                'Setpoint P_arranque / P_paro',
                <span style={M}>
                  {fmtBar(Pon_bar)} / {fmtBar(Poff_bar)} bar
                </span>,
              ],
              [
                'Tanque acumulador',
                <span style={M}>
                  {fmtL(Vt)} L · Precarga N₂ = {fmtBar(PN2_bar)} bar
                </span>,
              ],
              [
                'DN succión / impulsión',
                <span style={M}>
                  DN {rSucColector.dn} / DN {rImpColector.dn} mm (PVC Sch 40)
                </span>,
              ],
              [
                'Control',
                <span style={M}>VFD × {nt + dec(ep.nr)} · Transductor 4–20 mA · PLC/SCADA</span>,
              ],
              [
                'Normas',
                <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--txt4)' }}>
