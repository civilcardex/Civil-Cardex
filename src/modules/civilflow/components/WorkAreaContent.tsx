...
import { buildLlBajanteAssociations, computeLlQMap, computeLlRows, getTributarioIds } from "../utils/rainwaterRows";
...
async function downloadAnexoPdf(): Promise<void> {
  ...
  const doc = new jsPDF({ orientation: outCanvas.width >= outCanvas.height ? 'landscape' : 'portrait', unit: 'px', format: [outCanvas.width, outCanvas.height] });
...
const WorkAreaContent_S2: React.CSSProperties = { display: 'flex', gap: 12, alignItems: 'baseline', padding: '6px 10px', background: 'var(--bg3)', borderRadius: 'var(--r)', border: '1px solid var(--line)', marginBottom: 4 };
...
const WorkAreaContent_dlChip: React.CSSProperties = {
  display: 'flex', alignItems: 'center', gap: 10, padding: '10px 2px', borderBottom: '1px solid var(--line)', flexWrap: 'wrap',
};
const WorkAreaContent_dlLabel: React.CSSProperties = {
  flex: 1, fontSize: 14, fontWeight: 600, color: 'var(--txt)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: 130,
};
const WorkAreaContent_dlBtn: React.CSSProperties = {
  flexShrink: 0, padding: '7px 16px', borderRadius: 'var(--r)', border: '1px solid var(--line)',
  background: 'var(--bg2)', color: 'var(--txt)', fontSize: 13, fontWeight: 700, transition: 'filter .15s',
};
...
function InfTab({ state }: { state: WorkAreaState }) {
  ...
  const { proy, redesActivas, pisos, tramosSan, tramosLl, tramosAf, tramosAc, plans, aps, udBase, confirmedPlanos } = state;
  ...
  const okAF = tramosAf.length > 0 && tramosAf.every(t => t.velCumple !== false && t.presionOk !== false);
  ...
  const allOk = hasCheckedNet && (!hasSan || okSAN) && (!hasLl || okLL) && (!hasAf || okAF) && (!hasAc || okAC);
  const items = useMemo<[string, string][]>(() => {
    const rows: [string, string][] = [
      ['PROYECTO', proy.nombre],
      ['DIRECCIÓN/SECTOR', proy.dir || '—'],
      ['USO', proy.uso],
  ...
  const estadoLabel = !hasCheckedNet ? '—' : allOk ? '✓ Listo para descargar' : '✗ Revisar redes con errores';
  const estadoColor = !hasCheckedNet ? 'var(--txt3)' : allOk ? 'var(--ok)' : 'var(--err)';

  const f2 = (n: number) => n > 0 ? n.toFixed(2) : '—';

  // ── SAN ──
  const sanMergedBase = useMemo(() => SAN_UC_IDS.map(id => {
  ...
  const sanTable = useMemo<MemoriaTable | null>(() => {
    ...
    return {
      ...
      headerGroups: ['Tramo', 'Nivel',
        { label: 'Unidades de descarga', span: 3 },
        'No. Descargas', 'K', 'Caudal (LPS)', 'Manning', 'Pendiente (%)',
        { label: 'Diámetro', span: 3 },
        'Qo (LPS)', 'Vo (m/s)', 'Q/Qo', 'Velocidad real (m/s)', 'Chequeo velocidad',
        'Yc (mm)', 'Yn (mm)', 'Froude', 'Flujo', 'Ymax (mm)', 'Yn vs Yc',
        { label: 'Fuerza Tractiva', span: 2 }],
      headers: ['Tramo', 'Nivel', 'Propia', 'Otros', 'Total', 'No. Descargas', 'K', 'Caudal (LPS)', 'Manning', 'Pendiente (%)', 'Calculado (")', 'Diseño (")', 'Interior (mm)', 'Qo (LPS)', 'Vo (m/s)', 'Q/Qo', 'Velocidad real (m/s)', 'Chequeo velocidad', 'Yc (mm)', 'Yn (mm)', 'Froude', 'Flujo', 'Ymax (mm)', 'Yn vs Yc', 'Real (kg/m²)', '>0.15'],
      rows: rows.map(r => [
        r.id, pisoLbl(r.piso), r.udPropias, Math.max(0, r.udAcum - r.udPropias), r.udAcum, r.nSalidas || '—', r.K != null ? r.K.toFixed(2) : '—',
        r.Q != null && r.Q > 0 ? r.Q.toFixed(2) : '—', r.n > 0 ? r.n.toFixed(3) : '—', r.sVal > 0 ? r.sVal : '—',
        r.DcalcPulg > 0 ? r.DcalcPulg.toFixed(2) : '—', r.DdisPulg > 0 ? r.DdisPulg : '—', r.DintMm > 0 ? r.DintMm : '—',
        f2(r.Qo), f2(r.Vo), f2(r.qqo), f2(r.Vreal), r.chequeoV,
        f2(r.Yc), f2(r.Yn), f2(r.Froude), r.tipoFlujo, f2(r.Ymax), r.chequeoYn, f2(r.fuerzaTractiva), r.chequeoFT,
      ]),
    };
  }, [hasSan, tramosSan, plans, sanMergedBase]);
  ...
  const llTable = useMemo<MemoriaTable | null>(() => {
    ...
    return {
      ...
      headerGroups: ['Tramo', 'Nivel', 'Inicio', 'Fin', 'Bajantes asociados', 'Caudal (LPS)', 'Manning', 'Pendiente (%)',
        { label: 'Diámetro', span: 3 },
        'Qo (LPS)', 'Vo (m/s)', 'Q/Qo', 'V. real (m/s)', 'Chequeo velocidad',
        'Yc (mm)', 'Yn (mm)', 'Froude', 'Flujo', 'Ymax (mm)', 'Yn vs Yc',
        { label: 'Fuerza Tractiva', span: 2 }],
      headers: ['Tramo', 'Nivel', 'Inicio', 'Fin', 'Bajantes asociados', 'Caudal (LPS)', 'Manning', 'Pendiente (%)', 'Calculado (")', 'Diseño (")', 'Interior (mm)', 'Qo (LPS)', 'Vo (m/s)', 'Q/Qo', 'V. real (m/s)', 'Chequeo velocidad', 'Yc (mm)', 'Yn (mm)', 'Froude', 'Flujo', 'Ymax (mm)', 'Yn vs Yc', 'Real (kg/m²)', '>0.15'],
      rows: rows.map(r => [
        r.id, r.piso ? pisoLbl(r.piso) : '—', r.desde || '—', r.hasta || '—', r.bajantesAsociadas.join(', ') || '—',
        r.Q > 0 ? r.Q.toFixed(2) : '—', r.n > 0 ? r.n.toFixed(3) : '—', r.sVal > 0 ? r.sVal : '—',
        r.DcalcPulg > 0 ? r.DcalcPulg.toFixed(2) : '—', r.DdisPulg > 0 ? r.DdisPulg : '—', r.DintMm > 0 ? r.DintMm : '—',
        f2(r.Qo), f2(r.Vo), f2(r.qqo), f2(r.Vreal), r.chequeoV,
        f2(r.Yc), f2(r.Yn), f2(r.Froude), r.tipoFlujo, f2(r.Ymax), r.chequeoYn, f2(r.fuerzaTractiva), r.chequeoFT,
      ]),
  ...
  const llCanalTable = useMemo<MemoriaTable | null>(() => {
    ...
    const headers = ['Ramal', 'Área parcial (m²)', 'Área acum. (m²)', 'Intensidad (mm/hr)', 'Coef. escorrentía', 'Caudal real (LPS)', 'Manning', 'Pendiente (%)', 'Base (cm)', 'Altura (cm)', 'Borde libre (cm)', 'Total (cm)', 'Caudal máximo (LPS)', 'Chequeo'];
    const rows = canalesLl.map(c => {
      const { Qreal, Qmax, chequeo, totalStr } = chequeoCanalLluvia(c);
      return [
        c.sector || '—', c.areaParcial || '—', c.areaAcumulada || '—', c.intensidad || '—', c.coeficienteC || '—',
        Qreal > 0 ? Qreal.toFixed(2) : '—', c.manning || '—', c.pendiente, c.b, c.h, 10, totalStr,
        Qmax > 0 ? Qmax.toFixed(2) : '—', chequeo,
      ];
    });
  ...
  const afTable = useMemo<MemoriaTable | null>(() => {
    if (!hasAf) return null;
    const rows = computeWaterNetworkRows('af', tramosAf, tramosAf, plans, proy.p_red, DIAMETROS_AF, lookupInterno as (pulg: number) => number);
    if (rows.length === 0) return null;
    return {
      title: 'Diseño de red agua fría',
      headerGroups: ['Tramo', 'Inicio', 'Final', 'Piso',
        { label: 'Unidades Consumo', span: 3 },
        'No. de descargas', 'K', 'Caudal (lps)', 'Diámetro estimado',
        { label: 'Diámetro', span: 2 },
        'Coeficiente C', 'Vel. (mm/s)',
        { label: 'Longitud (m)', span: 4 },
        { label: 'Pérdidas por fricción', span: 2 },
        { label: 'Presión', span: 2 }],
      headers: ['Tramo', 'Inicio', 'Final', 'Piso', 'Propia', 'Otros Ramales', 'Total', 'No. de descargas', 'K', 'Caudal (lps)', 'Diámetro estimado', 'Diseño', 'Interno', 'Coeficiente C', 'Vel. (mm/s)', 'Horizontal', 'Vertical', 'Eq. Accesorios', 'Total', '%', 'm', 'Inicial', 'Final'],
      rows: rows.map(r => [
        r.id, r.ini, r.fin, pisoLbl(r.piso),
        f2(r.udPropia), f2(Math.max(0, r.udTotal - r.udPropia)), f2(r.udTotal),
        r.nDesc > 0 ? String(r.nDesc) : '—',
        r.K > 0 ? r.K.toFixed(2) : '—',
        r.Qprob > 0 ? r.Qprob.toFixed(3) : '—',
        f2(r.diamEst), r.diamDis, f2(r.dInt),
        String(r.cHW), f2(r.Vmms),
        f2(r.Lh), f2(r.Lv), f2(r.Le), f2(r.Lt),
        f2(r.hfPct), f2(r.hfM), f2(r.Pin), f2(r.Pfin),
      ]),
    };
  ...
  function buildAcometidaTables(): MemoriaTable[] {
    ...
    const tramosTable: MemoriaTable = {
      title: 'Acometida — tramos',
      headers: ['Tramo', 'Desde', 'Hasta', 'Horizontal (m)', 'Eq. Accesorios (m)', 'Diámetro estimado', 'Diámetro propuesto'],
      rows: [
        ['AC-01', d.tr1.desde, d.tr1.hasta, fmtAco(d.tr1.h), fmtAco(d.tr1.le), fmtAco(d.tr1.diamEstimado), d.tr1.diamPropuesto || '—'],
        ['AC-02', d.tr2.desde, d.tr2.hasta, fmtAco(d.tr2.h), fmtAco(d.tr2.le), fmtAco(d.tr2.diamEstimado), d.tr2.diamPropuesto || '—'],
      ],
    };
    const parametrosTable: MemoriaTable = {
      title: 'Acometida — resumen de parámetros',
    ...
    const verificacionTable: MemoriaTable = {
      ...
      rows: [
        ...
        ['Diámetro acometida vs contador', d.diamConformeOk ? 'Conforme' : `No conforme (+${fmtAco(d.diamDiff)}")`],
  ...
  const acTable = useMemo<MemoriaTable | null>(() => {
    if (!hasAc) return null;
    const rows = computeWaterNetworkRows('ac', tramosAc, tramosAf, plans, proy.p_red, DIAMETROS_AC, lookupInternoAC as (pulg: number) => number);
    if (rows.length === 0) return null;
    return {
  ...
  const gasTable = useMemo<MemoriaTable | null>(() => {
    ...
    return {
      ...
      headers: ['Tramo', 'Nivel', 'Inicio', 'Final', 'Material', 'DN', 'D int (mm)', 'K', 'Long (m)', 'Le (m)', 'ΔP (mbar)', 'Vel (m/s)', 'Pres. ini (mbar)', 'Pres. fin (mbar)', 'Chequeo'],
      rows: rows.map(r => [
        r.id, pisoLbl(r.piso), r.ini, r.fin,
        r.material || '—', r.dn || '—',
        r.dInt > 0 ? r.dInt.toFixed(2) : '—',
        r.K > 0 ? String(r.K) : '—',
        r.longitud > 0 ? r.longitud.toFixed(2) : '—',
        r.le.toFixed(2), r.dP.toFixed(2), r.vel.toFixed(2),
        r.pIni.toFixed(2), r.pFin.toFixed(2), r.chequeo,
      ]),
    };
  }, [hasGas, plans]);
  ...
  const handleDescargarMemoria = async () => {
    ...
    try {
      ...
      const tables = [
        ...
        ...tag([ucAfTable, afTable, ...buildAcometidaTables(), accAfTable], 'af'),
  ...
  return (
    <div className="fu" style={{ display: 'flex', flexDirection: 'column', gap: 12, flex: 1, minHeight: 0 }}>
      ...
      <section className="card" style={{ maxWidth: 460, width: '100%' }}>
        <div className="card-h">
          <h2 className="card-t">
            <img src="/iconos_civilflow/descargas.webp" alt="Descargas"  width={20} height={20} style={{width:20,height:20, verticalAlign: 'middle', marginRight: 4 }}  loading="lazy" />
            Descargas
          </h2>
          <span style={{
            display: 'inline-flex', alignItems: 'center', gap: 6, padding: '2px 8px', borderRadius: 999,
            fontSize: 10, fontWeight: 700, letterSpacing: 0.2,
            background: !hasCheckedNet ? 'var(--bg2)' : allOk ? 'rgba(34,197,94,.15)' : 'rgba(239,68,68,.15)',
            color: estadoColor, border: `1px solid ${!hasCheckedNet ? 'var(--line)' : allOk ? 'var(--ok)' : 'var(--err)'}`,
          }}>
            {estadoLabel}
          </span>
        </div>
        <div className="card-b" style={{ display: 'flex', flexDirection: 'column', padding: '4px 10px' }}>
          <div style={WorkAreaContent_dlChip}>
            <img src="/iconos_civilflow/memorias_finales.webp" alt=""  width={20} height={20} style={{width:20,height:20,flexShrink:0}}  loading="lazy" />
            <span style={WorkAreaContent_dlLabel}>Memorias finales</span>
            <div style={{ display: 'flex', alignItems: 'stretch', borderRadius: 'var(--r)', overflow: 'hidden', border: '1px solid var(--line)', opacity: allOk ? 1 : 0.5, flexShrink: 0 }}>
              <select
              ...
              <button type="button"
                ...
                style={{
                  padding: '6px 14px', fontSize: 13, fontWeight: 700, border: 'none', cursor: allOk ? 'pointer' : 'not-allowed',
                  background: allOk ? 'var(--acc)' : 'var(--bg2)', color: allOk ? '#0f1115' : 'var(--txt3)',
                }}
              >
          ...
          <div style={WorkAreaContent_dlChip}>
            <img src="/iconos_civilflow/detalle_aparatos.webp" alt=""  width={20} height={20} style={{width:20,height:20,flexShrink:0}}  loading="lazy" />
          ...
          <div style={WorkAreaContent_dlChip}>
            <img src="/iconos_civilflow/planos_red.webp" alt=""  width={20} height={20} style={{width:20,height:20,flexShrink:0}}  loading="lazy" />
