-- Campos de copia entre pisos y marcadores que el cliente envía pero el RPC vivo descartaba:
-- factor_sim/longitud (cf_planos_bajantes) tenían columnas desde 20260805000002/20260825000001
-- pero las recreaciones del RPC (20260910140000, 20260912000000) las dejaron fuera del
-- INSERT/UPDATE → siempre NULL al recargar. copia_piso/copiado_de_plan/copiado_de_id/
-- bajante_externo_id/sin_acc_med_interior no tenían columna (bloqueo de movimiento de copias
-- y enlace manual canal↔bajante externo se perdían en el round-trip).
-- APLICAR EN SQL EDITOR ANTES de desplegar la app. Idempotente.
-- UP
alter table public.cf_planos_ramales add column if not exists copia_piso boolean;
alter table public.cf_planos_ramales add column if not exists sin_acc_med_interior boolean;
alter table public.cf_planos_bajantes add column if not exists copia_piso boolean;
alter table public.cf_planos_bajantes add column if not exists copiado_de_plan text;
alter table public.cf_planos_bajantes add column if not exists copiado_de_id text;
alter table public.cf_planos_bajantes add column if not exists bajante_externo_id text;
...
begin
  ...
  select p_plano_id, uid, r.client_id, coalesce(r.x1, 0), coalesce(r.x2, 0), coalesce(r.y1, 0), coalesce(r.y2, 0), coalesce(r.l, 0), r.lbl_x, r.lbl_y
...
-- DOWN
alter table public.cf_planos_ramales drop column if exists copia_piso;
alter table public.cf_planos_ramales drop column if exists sin_acc_med_interior;
alter table public.cf_planos_bajantes drop column if exists copia_piso;
alter table public.cf_planos_bajantes drop column if exists copiado_de_plan;
alter table public.cf_planos_bajantes drop column if exists copiado_de_id;
alter table public.cf_planos_bajantes drop column if exists bajante_externo_id;
