-- FIX: save_plano_data transponía y1↔x2 de las COTAS (dimensiones) en cada guardado.
-- 20260912000000 y 20260914000000 recrearon el RPC con el SELECT de dimensiones cruzado:
--   columnas (x1, y1, x2, y2) ← valores (r.x1, r.x2, r.y1, r.y2)
-- → toda cota guardada a BD quedaba rotada 90° al recargar (el árbitro local-vs-BD entregaba
-- la fila transpuesta cuando la caché local no existía). Esta versión restaura el orden
-- (r.x1, r.y1, r.x2, r.y2). Idempotente; cuerpo idéntico a 20260914000000 salvo esa línea.
-- APLICAR EN SQL EDITOR — las filas ya transpuestas se reparan re-guardando cada piso desde la
-- app (la caché local conserva las cotas correctas y el autosave reescribe la BD sana).
-- UP
...
-- DOWN: la versión transpuesta era el defecto; sin DOWN.
