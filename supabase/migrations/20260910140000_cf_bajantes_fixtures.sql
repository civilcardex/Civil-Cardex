-- Fixtures (UDs por aparato) también en BAJANTES/bombas/cajas: antes solo los ramales
-- persistían sus claves de aparatos y las de bajante eran irrecuperables tras un borrado
-- local (orig. usuario: aparatos a 0 al reentrar). Requiere 20260910120000 aplicado antes
-- (la función recreada aquí incluye esas columnas también).
-- UP
alter table public.cf_planos_bajantes add column if not exists fixtures jsonb not null default '{}';
...
-- DOWN
alter table public.cf_planos_bajantes drop column if exists fixtures;
