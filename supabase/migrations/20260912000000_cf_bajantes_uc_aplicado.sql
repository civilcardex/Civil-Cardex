-- Libro de herencia entre pisos (orig. usuario: fantasma y bajante original en 0 tras
-- reentrar, LDesvio bien): `uc_aplicado`/`uc_aplicado_hidro` solo vivían en la caché local
-- — un round-trip BD (árbitro por ts) los borraba de motor y caché. Viajan ahora en la fila
-- del bajante; get_plano_data no cambia (devuelve to_jsonb con todas las columnas).
-- Converge el RPC: re-añade caja_origen_id/bomba_en_id (20260910120000) que el cuerpo de
-- 20260910140000 dejó fuera, más fixtures y las dos columnas nuevas.
-- Requiere 20260910140000 aplicado antes. APLICAR EN SQL EDITOR ANTES de desplegar la app.
-- UP
alter table public.cf_planos_bajantes add column if not exists uc_aplicado jsonb;
alter table public.cf_planos_bajantes add column if not exists uc_aplicado_hidro jsonb;
...
-- DOWN
alter table public.cf_planos_bajantes drop column if exists uc_aplicado;
alter table public.cf_planos_bajantes drop column if exists uc_aplicado_hidro;
