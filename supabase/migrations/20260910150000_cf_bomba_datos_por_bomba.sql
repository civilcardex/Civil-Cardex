-- Cálculos POR BOMBA (orig. usuario): mapa código de bomba → inputs propios (salidas,
-- alturas, diámetro, tipo de tubería, etc.). tipo_tuberia global queda como default.
-- UP
alter table public.cf_bomba_datos_proyecto add column if not exists bombas jsonb not null default '{}';
...
begin
  ...
  select p_proyecto_id, uid, r.sal_sim, r.ud_tot, r.hz, r.l_imp, r.d_imp, r.c_hw, r.p_desc, r.eta_b, r.f_srv, r.t_cic, r.h_min, r.h_max, r.b_cam, r.l_cam, r.npsh, coalesce(nullif(r.tipo_tuberia, ''), 'PVC-PR'), coalesce(r.bombas, '{}'::jsonb), now()
...
-- DOWN
alter table public.cf_bomba_datos_proyecto drop column if exists bombas;
