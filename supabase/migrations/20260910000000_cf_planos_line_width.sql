-- Grosor de líneas del dibujo por plano (slider de la barra de redes activas, orig. usuario).
-- cf_planos.line_width alimenta el factor multiplicador de todos los lineWidth del visor
-- (engine.lineWidthScale); get_plano_data ya devuelve todas las columnas (to_jsonb), no cambia.
-- UP
alter table public.cf_planos add column if not exists line_width numeric not null default 1;
...
-- DOWN
alter table public.cf_planos drop column if exists line_width;
