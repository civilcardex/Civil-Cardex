-- Flag de etiqueta movida a mano (auto-orden solo reubica las nunca movidas).
-- UP
alter table public.cf_planos_ramales add column if not exists label_moved boolean default false;
alter table public.cf_planos_bajantes add column if not exists label_moved boolean default false;
alter table public.cf_planos_areas add column if not exists label_moved boolean default false;

-- Actualizar RPC save_plano_data para incluir las nuevas columnas
-- (get_plano_data usa to_jsonb y las trae automático, no requiere cambio).
...
-- DOWN
-- conservar columnas (requeridas por UI)
select 1;
