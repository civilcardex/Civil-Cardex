-- Canal longitud persistencia (ítem 13)
-- La tabla cf_planos_bajantes tenía base y altura pero no longitud → el largo del canal
-- se perdía al guardar vía save_plano_data (ruta DB). Esta migración añade la columna
-- y parchea el RPC save_plano_data para incluirla.

alter table public.cf_planos_bajantes add column if not exists longitud numeric;

-- Compatibilidad: si alguna instalación antigua aún tiene planos_bajantes sin prefijo
do $$ begin
  if exists (select 1 from information_schema.tables where table_schema='public' and table_name='planos_bajantes') then
    execute 'alter table public.planos_bajantes add column if not exists longitud numeric';
  end if;
end $$;
...
begin
  ...
  with ins as (
    insert into public.cf_planos_bajantes (plano_id, user_id, client_id, net, tipo, code, x, y, piso_base, piso_cima, npt_base, npt_cima, h_vert, d_nominal, uc_acum, uc_extra, area_m2, desplazamientos, lbl_off_x, lbl_off_y, label_angle, label_x, label_y, direccion, aparato, total_l, pendiente, piso, baj_r, ghost_data, is_fantasma, diam_pulg, diametro, aco_diam, capacidad, base, altura, longitud, canal_id, descarga_en_id, origen_id)
    select p_plano_id, uid, r.client_id, r.net, r.tipo, r.code, coalesce(r.x, 0), coalesce(r.y, 0), r.piso_base, r.piso_cima, r.npt_base, r.npt_cima, r.h_vert, r.d_nominal, r.uc_acum, r.uc_extra, r.area_m2, coalesce(r.desplazamientos, '{}'::jsonb), r.lbl_off_x, r.lbl_off_y, r.label_angle, r.label_x, r.label_y, r.direccion, r.aparato, r.total_l, r.pendiente, r.piso, r.baj_r, r.ghost_data, coalesce(r.is_fantasma, false), r.diam_pulg, r.diametro, r.aco_diam, r.capacidad, r.base, r.altura, r.longitud, r.canal_id, r.descarga_en_id, r.origen_id
